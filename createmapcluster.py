...
def parse_arguments() -> str:
    ...
    parser = argparse.ArgumentParser(
        description="Create a map from a CSV file of locations."
    )
    parser.add_argument(
        "csv_file", help="Path to the CSV file containing location data"
    )
...
def get_coordinates(
    address: str, geolocator: Nominatim, max_retries: int = 3
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(
                f"Geocoding attempt {attempt + 1} failed, waiting {wait_time:.2f} seconds..."
            )
...
def process_coordinates(
    df: pd.DataFrame, geolocator: Nominatim, csv_path: str
) -> pd.DataFrame:
    ...
    for index, row in df.iterrows():
        ...
        if pd.notna(row.get("Latitude")) and pd.notna(row.get("Longitude")):
            print(
                f"Skipping row {index} - coordinates already exist: {row['Latitude']}, {row['Longitude']}"
            )
            continue
...
def check_boundaries(df: pd.DataFrame, boundary_polygon: Polygon) -> None:
    ...
    for index, row in df.iterrows():
        ...
        if not is_within_boundary(row["Latitude"], row["Longitude"], boundary_polygon):
            print(
                f"Warning: Coordinates ({row['Latitude']}, {row['Longitude']}) for address '{row['Address Line 1']}, {row['City']}, {row['State']} {row['Zip']}' are outside the boundary."
            )
        ...
        try:
            ...
            folium.map.Marker(
                ...
                icon=folium.features.DivIcon(
                    icon_size=(150, 36), icon_anchor=(0, 0), html=html_text
                ),
